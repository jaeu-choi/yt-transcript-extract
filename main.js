...
// IPC 핸들러 추가 - 프론트에서 서버 포트 요청시
ipcMain.on("request-server-port", (event) => {
  console.log("📨 Frontend requested server port");
  if (serverPort) {
    console.log(`📡 Sending port ${serverPort} to frontend (via IPC request)`);
    event.reply("server-port", serverPort);
  } else {
    console.log("⚠️ Server port not ready yet");
  }
});
    ...
    try {
      ...
      pythonProcess.stdout.on("data", (data) => {
        ...
        if (portMatch) {
          ...
          // 포트가 감지되면 서버 시작으로 간주하고 바로 resolve
          if (!serverStarted) {
            serverStarted = true;
            clearTimeout(timeout);
